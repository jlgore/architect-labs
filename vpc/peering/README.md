...
Before we can launch EC2 instances, we need to create a key pair that will allow us to securely connect to our instances via SSH. The private key will be downloaded to your local machine, and the public key will be stored in AWS.

```bash
# Create a key pair for SSH access
aws ec2 create-key-pair --key-name vpc-peering-key --query "KeyMaterial" --output text > vpc-peering-key.pem

# Set correct permissions for the private key file
chmod 400 vpc-peering-key.pem

echo "Key pair 'vpc-peering-key' has been created and the private key has been saved as vpc-peering-key.pem"
```
...
### 5. Store Subnet IDs in Variables
...
### 6. Enable Auto-assign Public IP for Subnets
...
### 9. Create a VPC Peering Connection
...
### 10. Configure Route Tables for Peering
...
### 12. Get the Latest Amazon Linux AMI ID
...
### 13. Launch EC2 Instances in Each VPC

Now we'll launch EC2 instances in each VPC to test the peering connection. We'll use t2.micro instances (which are free tier eligible and sandbox-compatible) running the latest Amazon Linux 2023 AMI and our newly created key pair. We'll explicitly associate public IPs with our instances for SSH access.
...
### 14. Wait for Instances to be Running
...
echo "ssh -i vpc-peering-key.pem ec2-user@$INSTANCE_A_PUBLIC_IP 'ping -c 4 $INSTANCE_B_PRIVATE_IP'"
```
...
# Delete the key pair
aws ec2 delete-key-pair --key-name vpc-peering-key

# Finally, delete the VPCs themselves
aws ec2 delete-vpc --vpc-id $VPC_A_ID
aws ec2 delete-vpc --vpc-id $VPC_B_ID

echo "Don't forget to delete the vpc-peering-key.pem file from your local machine"
```
